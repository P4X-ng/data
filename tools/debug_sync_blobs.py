...
"""
        ...
        try:
            ...
            rsync_result = subprocess.run([
                'rsync', '-avz', tmp_path, f'punk@{self.remote_host}:/tmp/remote_blob_gen.py'
    ...
    def compare_blobs(self, local_blob: bytes, remote_blob: bytes):
        ...
        if not data_match:
            ...
            # Count differences
            diff_count = sum(1 for i in range(min_len) if local_blob[i] != remote_blob[i])
            diff_percentage = (diff_count / min_len) * 100 if min_len > 0 else 0
            
            print(f"   Differences: {diff_count}/{min_len} bytes ({diff_percentage:.2f}%)")
    ...
    def run_sync_debug(self, blob_size: int = 65536):
        ...
        try:
            ...
            if not blobs_match:
                ...
                print(f"   Fix: Ensure identical blob generation on both architectures.")
