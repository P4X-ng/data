...
class ChecksumDebugger:
    ...
    def create_test_pattern_file(self, pattern_type: str, size_kb: int = 64) -> str:
        ...
        self.logger.info(f"Creating {pattern_type} pattern file: {filename} ({size_kb}KB)")
        ...
        with open(filename, 'wb') as f:
            if pattern_type == "all_A":
                ...
                f.write(data)
                self.logger.debug(f"Pattern: Little-endian 32-bit ints, size={len(data)}")
        ...
        self.logger.info(f"Created {filename}: {os.path.getsize(filename)} bytes, SHA256: {file_hash[:16]}...")
    ...
    def debug_packetfs_transfer(self, test_file: str) -> Dict:
        ...
        self.logger.info(f"Original file: {original_size} bytes, SHA256: {original_hash[:16]}...")
        ...
        self.logger.debug(f"Original first chunk (64 bytes):\n{self.hex_dump(first_chunk)}")
        self.logger.debug(f"Original last chunk (64 bytes):\n{self.hex_dump(last_chunk)}")
        ...
        self.logger.info(f"Received file: {received_size} bytes, SHA256: {received_hash[:16]}...")
        ...
        self.logger.debug(f"Received first chunk (64 bytes):\n{self.hex_dump(received_first_chunk)}")
        self.logger.debug(f"Received last chunk (64 bytes):\n{self.hex_dump(received_last_chunk)}")
        ...
        self.logger.info(f"Size match: {size_match} ({original_size} vs {received_size})")
        ...
        if not first_chunk_match:
            for i, (a, b) in enumerate(zip(first_chunk, received_first_chunk)):
                if a != b:
                    self.logger.warning(f"First difference at byte {i}: original=0x{a:02x}, received=0x{b:02x}")
                    break
                    
        # Calculate throughput
        throughput_mbs = original_size / (1024 * 1024) / transfer_time if transfer_time > 0 else 0
        ...
        self.logger.info(f"✅ Transfer completed: {throughput_mbs:.2f} MB/s, Integrity: {'✅' if hash_match else '❌'}")
    ...
    def run_checksum_debug_suite(self):
            ...
            for size_kb in sizes:
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        print(f"  {pattern:>12} {size_kb:>3}KB: {result['throughput_mbs']:>6.2f} MB/s, "
                              f"Integrity: {integrity}, Size: {result['size_match']}")
                    else:
                        print(f"  {pattern:>12} {size_kb:>3}KB: ❌ FAILED - {result.get('error', 'Unknown')}")
        ...
        if successful_tests:
            integrity_rate = sum(1 for r in successful_tests if r["hash_match"]) / len(successful_tests) * 100
            avg_throughput = sum(r["throughput_mbs"] for r in successful_tests) / len(successful_tests)
            ...
            for pattern, results_list in pattern_results.items():
                integrity_rate = sum(1 for r in results_list if r["hash_match"]) / len(results_list) * 100
                print(f"  {pattern:>12}: {integrity_rate:>5.1f}% ({sum(1 for r in results_list if r['hash_match'])}/{len(results_list)})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--remote', default='10.69.69.235', help='Remote host for testing')
    parser.add_argument('--pattern', choices=['all_A', 'all_zero', 'sequential', 'alternating', 'endian_test'],
                       help='Test specific pattern only')
    parser.add_argument('--size', type=int, default=64, help='File size in KB (default: 64)')
