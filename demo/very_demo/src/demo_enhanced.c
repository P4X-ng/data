...
int main(int argc, char* argv[]) {
    ...
    // Validate filesystem size with overflow-safe calculation
    uint64_t total_size_bytes = (uint64_t)fs_size_gb * 1024ULL * 1024ULL * 1024ULL;
    size_t minimum_bytes = sizeof(PacketFSSuperblock) + sizeof(PacketFSNode);
    if (fs_size_gb <= 0 || total_size_bytes < (uint64_t)minimum_bytes) {
        fprintf(stderr,
                "Error: filesystem size must be >= %zu bytes (%zu KB). Got %lld GB.\n",
                minimum_bytes, minimum_bytes / 1024, (long long)fs_size_gb);
        fprintf(stderr, "Tip: use at least 1 GB for demos.\n");
        exit(EXIT_FAILURE);
    }
    ...
    char* read_data;
    rc = posix_memalign((void**)&read_data, 64, file_size);
    if (rc != 0) {
        errno = rc;
        perror("posix_memalign failed for read data");
        free(test_data);
        cleanup_packetfs(pfs);
        return 1;
    }
