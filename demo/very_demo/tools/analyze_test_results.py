...
class PacketFSResultsAnalyzer:
    ...
    def generate_performance_plots(self, output_dir: str = "analysis_plots"):
        ...
        print(f"✅ Generated {len(list(output_path.glob('*.png')))} visualization plots")
    ...
    def _plot_throughput_vs_payload(self, filename: str):
        ...
        payload_groups = self.df.groupby('payload_size')[throughput_col].agg(['mean', 'std', 'max'])
        
        plt.errorbar(payload_groups.index, payload_groups['mean'], yerr=payload_groups['std'], 
                    marker='o', linewidth=2, markersize=8, capsize=5)
        plt.scatter(payload_groups.index, payload_groups['max'], marker='^', s=100, 
                   alpha=0.7, label='Peak Performance')
        ...
        plt.plot([0, max_val], [0, max_val], 'r--', alpha=0.5, label='Perfect Efficiency')
    ...
    def _plot_latency_vs_payload(self, filename: str):
        ...
        if 'encode_latency_avg_us' in self.df.columns:
            ...
            latency_p95_col = 'encode_latency_p95_us' if 'encode_latency_p95_us' in self.df.columns else None
        else:
        ...
        # Main latency plot
        plt.subplot(2, 3, 1)
        payload_groups = self.df.groupby('payload_size')[latency_avg_col].agg(['mean', 'std', 'min', 'max'])
        
        plt.errorbar(payload_groups.index, payload_groups['mean'], yerr=payload_groups['std'],
                    marker='o', linewidth=2, markersize=8, capsize=5, label='Average ± Std')
        plt.fill_between(payload_groups.index, payload_groups['min'], payload_groups['max'],
        ...
        latency_data = [self.df[self.df['payload_size'] == ps][latency_avg_col].values for ps in payload_sizes]
        ...
        plt.subplot(2, 3, 4)
        throughput_col = 'encode_mbps' if 'encode_mbps' in self.df.columns else 'mbits_per_second'
        plt.scatter(self.df[latency_avg_col], self.df[throughput_col], 
                   c=self.df['payload_size'], cmap='viridis', alpha=0.7)
        ...
        payload_range = np.logspace(np.log10(self.df['payload_size'].min()), 
                                  np.log10(self.df['payload_size'].max()), 100)
        predicted_latency = coeffs[0] * np.log(payload_range) + coeffs[1]
        
        plt.scatter(self.df['payload_size'], self.df[latency_avg_col], alpha=0.6, label='Measured')
        plt.plot(payload_range, predicted_latency, 'r--', linewidth=2, 
        ...
        plt.subplot(2, 3, 6)
        if 'window_size' in self.df.columns:
            window_latency = self.df.groupby('window_size')[latency_avg_col].agg(['mean', 'std'])
            plt.errorbar(window_latency.index, window_latency['mean'], yerr=window_latency['std'],
    ...
    def _plot_fps_vs_payload(self, filename: str):
        ...
        fps_col = 'encode_fps' if 'encode_fps' in self.df.columns else 'frames_per_second'
        
        plt.subplot(2, 2, 1)
        payload_groups = self.df.groupby('payload_size')[fps_col].agg(['mean', 'std', 'max'])
        
        plt.errorbar(payload_groups.index, payload_groups['mean'], yerr=payload_groups['std'],
                    marker='o', linewidth=2, markersize=8, capsize=5, label='Average ± Std')
        plt.scatter(payload_groups.index, payload_groups['max'], marker='^', s=100,
                   alpha=0.7, label='Peak FPS')
        ...
        if 'test_name' in self.df.columns:
            ...
            plt.xticks(range(len(test_perf)), [f'{size}B' for size in test_perf.index], rotation=45)
    ...
    def _plot_window_size_impact(self, filename: str):
        ...
        plt.subplot(2, 3, 1)
        throughput_col = 'encode_mbps' if 'encode_mbps' in self.df.columns else 'mbits_per_second'
        window_throughput = self.df.groupby('window_size')[throughput_col].agg(['mean', 'std'])
        
        plt.errorbar(window_throughput.index, window_throughput['mean'], 
                    yerr=window_throughput['std'], marker='o', linewidth=2, 
        ...
        plt.subplot(2, 3, 2)
        latency_col = 'encode_latency_avg_us' if 'encode_latency_avg_us' in self.df.columns else 'avg_latency_us'
        window_latency = self.df.groupby('window_size')[latency_col].agg(['mean', 'std'])
        
        plt.errorbar(window_latency.index, window_latency['mean'],
                    yerr=window_latency['std'], marker='s', linewidth=2,
                    markersize=8, capsize=5)
        plt.xlabel('Window Size (bytes)')
        plt.ylabel('Latency (μs)')
        plt.title('Latency vs Window Size')
        ...
        plt.subplot(2, 3, 4)
        pivot_data = self.df.pivot_table(values=throughput_col, 
                                       index='payload_size', 
                                       columns='window_size', 
                                       aggfunc='mean')
    ...
    def _plot_bit_efficiency(self, filename: str):
        ...
        if 'window_size' in self.df.columns:
            efficiency_by_window = self.df.groupby('window_size')['bit_efficiency'].agg(['mean', 'std'])
            plt.errorbar(efficiency_by_window.index, efficiency_by_window['mean'],
                        yerr=efficiency_by_window['std'], marker='o')
            plt.xlabel('Window Size (bytes)')
            plt.ylabel('Bit Efficiency')
            plt.title('Bit Efficiency vs Window Size')
            plt.grid(True, alpha=0.3)
    ...
    def _plot_performance_heatmap(self, filename: str):
        ...
        plt.subplot(2, 2, 1)
        pivot_throughput = self.df.pivot_table(values=throughput_col, 
                                             index='payload_size',
                                             columns='window_size' if 'window_size' in self.df.columns else 'frame_count',
                                             aggfunc='mean')
        sns.heatmap(pivot_throughput, annot=True, fmt='.1f', cmap='YlOrRd', cbar_kws={'label': 'Throughput (Mbps)'})
        plt.title('Throughput Performance Heatmap')
        plt.ylabel('Payload Size (bytes)')
        
        # Latency heatmap
        plt.subplot(2, 2, 2)
        pivot_latency = self.df.pivot_table(values=latency_col,
                                          index='payload_size', 
                                          columns='window_size' if 'window_size' in self.df.columns else 'frame_count',
                                          aggfunc='mean')
        sns.heatmap(pivot_latency, annot=True, fmt='.0f', cmap='YlOrRd_r', cbar_kws={'label': 'Latency (μs)'})
        ...
        plt.subplot(2, 2, 3)
        fps_col = 'encode_fps' if 'encode_fps' in self.df.columns else 'frames_per_second'
        pivot_fps = self.df.pivot_table(values=fps_col,
                                      index='payload_size',
                                      columns='window_size' if 'window_size' in self.df.columns else 'frame_count',
                                      aggfunc='mean')
        sns.heatmap(pivot_fps, annot=True, fmt='.0f', cmap='viridis', cbar_kws={'label': 'FPS'})
        ...
        norm_throughput = (self.df[throughput_col] - self.df[throughput_col].min()) / (self.df[throughput_col].max() - self.df[throughput_col].min())
        norm_latency = 1 - (self.df[latency_col] - self.df[latency_col].min()) / (self.df[latency_col].max() - self.df[latency_col].min())
        
        self.df['performance_score'] = (norm_throughput + norm_latency) / 2
        
        pivot_score = self.df.pivot_table(values='performance_score',
                                        index='payload_size',
                                        columns='window_size' if 'window_size' in self.df.columns else 'frame_count',
                                        aggfunc='mean')
        sns.heatmap(pivot_score, annot=True, fmt='.2f', cmap='RdYlGn', cbar_kws={'label': 'Performance Score'})
    ...
    def _plot_latency_distribution(self, filename: str):
        ...
        plt.subplot(2, 3, 2)
        payload_sizes = sorted(self.df['payload_size'].unique())
        latency_by_payload = [self.df[self.df['payload_size'] == ps][latency_col].values for ps in payload_sizes]
        ...
        latency_percentiles = [np.percentile(self.df[latency_col], p) for p in percentiles]
        ...
        plt.subplot(2, 3, 4)
        throughput_col = 'encode_mbps' if 'encode_mbps' in self.df.columns else 'mbits_per_second'
        plt.scatter(self.df[latency_col], self.df[throughput_col], 
                   c=self.df['payload_size'], cmap='viridis', alpha=0.7)
        plt.colorbar(label='Payload Size (bytes)')
        plt.xlabel('Latency (μs)')
        plt.ylabel('Throughput (Mbps)')
        ...
        plt.subplot(2, 3, 5)
        payload_groups = self.df.groupby('payload_size')[latency_col].agg(['mean', 'std', 'min', 'max'])
        
        plt.errorbar(payload_groups.index, payload_groups['mean'], yerr=payload_groups['std'],
                    marker='o', linewidth=2, markersize=6, capsize=3, label='Mean ± Std')
        plt.fill_between(payload_groups.index, payload_groups['min'], payload_groups['max'],
                        alpha=0.2, label='Min-Max Range')
        plt.xlabel('Payload Size (bytes)')
        plt.ylabel('Latency (μs)')
    ...
    def _plot_scaling_analysis(self, filename: str):
        ...
        plt.scatter(self.df['payload_size'], self.df[throughput_col], alpha=0.6, label='Measured')
        plt.plot(payload_range, predicted_throughput, 'r--', linewidth=2,
        ...
        plt.scatter(self.df['payload_size'], self.df[fps_col], alpha=0.6, label='Measured')
        plt.plot(payload_range, predicted_fps, 'r--', linewidth=2,
        ...
        predicted_latency = latency_coeffs[0] * np.log(payload_range) + latency_coeffs[1]
        
        plt.scatter(self.df['payload_size'], self.df[latency_col], alpha=0.6, label='Measured')
        plt.plot(payload_range, predicted_latency, 'r--', linewidth=2,
        ...
        plt.subplot(3, 3, 4)
        efficiency = self.df[throughput_col] / self.df[latency_col] * 1000  # Mbps per ms
        ...
        if 'frame_count' in self.df.columns:
            ...
            time_per_work = (self.df['frame_count'] / self.df[fps_col]) / total_work * 1e6  # μs per byte
        ...
        plt.subplot(3, 3, 6)
        predicted_throughput_validation = np.exp(coeffs[1]) * (self.df['payload_size'] ** coeffs[0])
        ...
        plt.subplot(3, 3, 7)
        if 'window_size' in self.df.columns:
            config_perf = self.df.groupby(['payload_size', 'window_size'])[throughput_col].mean().unstack()
        ...
        norm_throughput = payload_summary[throughput_col] / payload_summary[throughput_col].max()
        norm_fps = payload_summary[fps_col] / payload_summary[fps_col].max()
        norm_latency = 1 - (payload_summary[latency_col] / payload_summary[latency_col].max())
        
        x = range(len(payload_summary))
        width = 0.25
        
        plt.bar([i - width for i in x], norm_throughput, width, label='Throughput', alpha=0.8)
        plt.bar([i for i in x], norm_fps, width, label='FPS', alpha=0.8)
        plt.bar([i + width for i in x], norm_latency, width, label='Latency (inv)', alpha=0.8)
    ...
    def generate_comprehensive_report(self, output_file: str = "comprehensive_analysis_report.txt"):
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"{'Size (B)':<8} {'Avg Mbps':<10} {'Max Mbps':<10} {'Avg FPS':<12} {'Max FPS':<12} {'Avg Lat (μs)':<12}\n")
            f.write("-" * 70 + "\n")
            
            for size in sorted(self.df['payload_size'].unique()):
                size_data = self.df[self.df['payload_size'] == size]
                f.write(f"{size:<8} {size_data[throughput_col].mean():<10.2f} "
            ...
            if 'window_size' in self.df.columns:
                ...
                window_analysis = self.df.groupby('window_size').agg({
                    throughput_col: 'mean',
                    latency_col: 'mean',
                    'sync_frames': 'mean' if 'sync_frames' in self.df.columns else throughput_col
                })
                ...
                for window in sorted(self.df['window_size'].unique()):
                    ...
                    f.write(f"{window:<10} {window_data[throughput_col].mean():<10.2f} "
            ...
            f.write(f"Throughput: {np.exp(throughput_coeffs[1]):.2f} * payload_size^{throughput_coeffs[0]:.3f}\n")
            f.write(f"FPS: {np.exp(fps_coeffs[1]):.0f} * payload_size^{fps_coeffs[0]:.3f}\n")
            f.write(f"Latency: {latency_coeffs[0]:.2f} * ln(payload_size) + {latency_coeffs[1]:.2f}\n\n")
            ...
            throughput_per_latency = self.df[throughput_col] / (self.df[latency_col] / 1000)  # Mbps per ms
            fps_per_payload = self.df[fps_col] / self.df['payload_size']
            
            f.write(f"Best Throughput/Latency Ratio: {throughput_per_latency.max():.2f} Mbps/ms\n")
            f.write(f"Best FPS/Payload Ratio: {fps_per_payload.max():.2f} fps/byte\n")
            
            if 'bit_efficiency' in self.df.columns:
                f.write(f"Average Bit Efficiency: {self.df['bit_efficiency'].mean():.6f}\n")
            ...
            f.write(f"For Maximum Throughput ({best_throughput[throughput_col]:.2f} Mbps):\n")
            ...
            # Identify trends
            small_payload_perf = self.df[self.df['payload_size'] <= 128][throughput_col].mean()
            large_payload_perf = self.df[self.df['payload_size'] >= 1024][throughput_col].mean()
            
            f.write(f"1. Small payloads (≤128B) average: {small_payload_perf:.2f} Mbps\n")
            f.write(f"2. Large payloads (≥1024B) average: {large_payload_perf:.2f} Mbps\n")
            
            if large_payload_perf > small_payload_perf:
                f.write("3. Throughput increases with payload size (bulk transfer optimized)\n")
            else:
                f.write("3. Higher throughput with smaller payloads (low-latency optimized)\n")
            
            # Latency scaling insight
            latency_growth_rate = latency_coeffs[0]
            if latency_growth_rate > 0:
                f.write(f"4. Latency grows logarithmically with payload size (rate: {latency_growth_rate:.2f} μs/ln(byte))\n")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="PacketFS Test Results Analysis and Visualization")
    parser.add_argument("--results", nargs="+", default=[
        "simple_packetfs_results.json",
    ...
    try:
        ...
        analyzer.generate_comprehensive_report(f"{args.output_dir}/comprehensive_analysis_report.txt")
