...
class PacketFSLLVMKernel:
    ...
    def allocate_hugepage_memory(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['mount', '-t', 'hugetlbfs', 'nodev', '/mnt/hugepages'], 
                                  capture_output=True)
            if result.returncode != 0:
                # Try to create the mount point
                os.makedirs('/mnt/hugepages', exist_ok=True)
                subprocess.run(['mount', '-t', 'hugetlbfs', 'nodev', '/mnt/hugepages'], 
                              check=True)
            ...
            for i in range(self.hugepages_count):
                ...
                with open(hugepage_file, 'r+b') as f:
                    mm = mmap.mmap(f.fileno(), HUGEPAGE_SIZE, 
                                  mmap.MAP_SHARED, mmap.PROT_READ | mmap.PROT_WRITE)
                    self.hugepage_maps.append(mm)
                    
            self.metrics.hugepages_allocated = len(self.hugepage_maps)
            self.metrics.memory_used_mb = (self.metrics.hugepages_allocated * HUGEPAGE_SIZE) // (1024*1024)
        ...
        for shard_id in range(LLVM_PACKET_SHARDS):
            if current_offset + PACKET_SIZE > HUGEPAGE_SIZE:
                ...
                if current_hugepage >= len(self.hugepage_maps):
                    logger.warning(f"Reached hugepage limit, created {shards_created} shards")
                    break
    ...
    def llvm_optimize_shards(self) -> int:
        ...
        total_compression = sum(s.compression_ratio for s in self.packet_shards.values())
    ...
    def transfer_between_systems(self, target_system: 'PacketFSLLVMKernel', 
                                data_mb: int) -> Tuple[float, float]:
        ...
        logger.info(f"  Transfer rate: {transfer_rate_mbps:.1f} MB/s ({transfer_rate_gbps:.3f} GB/s)")
