...
def build_preface(transfer_id: str, channels: int, channel_id: int,
                  blob_fingerprint: str, object_sha256: str, psk: str | None = None,
                  flags: int = 0) -> bytes:
    ...
    hdr = [MAGIC_PREFACE, struct.pack("!BBBBH", VER, flags, (channels >> 8) & 0xFF, channels & 0xFF, channel_id)]
    # Note: pack CHANNELS into two separate bytes for readability above, then CH_ID as H
    # Correct it by packing as: channels H, channel_id H
...
def parse_preface(data: bytes) -> dict:
    ...
    return {"version": ver, "flags": flags, "channels": channels, "channel_id": ch_id,
            "transfer_id": tid, "blob": blob, "object": obj, "psk": psk}
