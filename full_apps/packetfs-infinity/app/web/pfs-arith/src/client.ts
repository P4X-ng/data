import { buildPreface, buildCtrlJsonFrame, buildFramesFromData, buildCtrlBinWin, buildCtrlBinEnd, buildCtrlBinDone } from './pvrtFraming';
import { buildContainer } from './pvrtContainer';
import { windowHashes } from './hash';
import type { IprogPlan, SendResult } from './types';
import type { WebSocketLike } from './wsAdapters';

export class PfsArithClient {
  constructor(private wsFactory: (url: string) => WebSocketLike) {}

  // Single-WS arithmetic send (uses binary WIN/END controls + relative BREF)
  async sendIprog(url: string, iprog: IprogPlan, transferId: string): Promise<SendResult> {
    ...
    const needed: number[] = Array.isArray(needMsg?.needed) ? needMsg.needed.map((x: any) => Number(x)) : iprog.windows.map(w => w.idx);

    // Send frames for needed windows (binary controls)
    const frames: Uint8Array[] = [];
    for (const w of iprog.windows) {
      if (!needed.includes(w.idx)) continue;
      ...
      if (payload && payload.length) frames.push(buildFramesFromData(payload, { windowSize: 1024 }));
  ...
  async sendIprogMulti(url: string, iprog: IprogPlan, transferId: string, channels = 4): Promise<SendResult> {
    if (channels <= 1) return this.sendIprog(url, iprog, transferId);
    const t0 = Date.now();
    const blob = iprog.blob;
    const fp = `${blob.name}:${blob.size}:${blob.seed}`;
    const anchor = BigInt(Math.floor((blob.size || 0) / 2));
    ...
    const wses: WebSocketLike[] = new Array(channels).fill(0).map(() => this.wsFactory(url));
    let bytesSent = 0;
    ...
    await wses[0].send(mf); bytesSent += mf.length;
    ...
    const buckets: number[][] = Array.from({ length: channels }, () => []);
    needed.forEach((widx, i) => { buckets[i % channels].push(widx); });

    function framesForBucket(bucket: number[]): Uint8Array {
      ...
      for (const idx of bucket) {
        ...
        if (payload && payload.length) out.push(buildFramesFromData(payload, { windowSize: 1024 }));
    ...
    await Promise.all(wses.map(async (ws, i) => {
      ...
      if (frames.length) { await ws.send(frames); bytesSent += frames.length; }
      if (i === 0) {
        const done = buildCtrlJsonFrame('DONE', { sha: iprog.sha256, tw: iprog.windows.length, ws: windowSize });
        await ws.send(done); bytesSent += done.length;
      }
    }));
...
function hexToBytes(hex: string): Uint8Array {
  ...
  for (let i = 0; i < out.length; i++) out[i] = parseInt(clean.substr(i * 2, 2), 16) & 0xff;
  return out;
}
...
function base64ToBytes(b64: string): Uint8Array {
  ...
  const text = decodeURIComponent(
    Array.prototype.map
      .call(b64.replace(/\s+/g, ''), (c: string) => `%${('00' + c.charCodeAt(0).toString(16)).slice(-2)}`)
      .join('')
  );
