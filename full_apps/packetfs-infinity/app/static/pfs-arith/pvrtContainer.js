...
export function buildContainer(opts) {
    ...
    if (opts.raw && opts.raw.length)
        sections.push({ type: SEC_RAW, data: opts.raw });
    if (opts.proto && opts.proto.length)
        sections.push({ type: SEC_PROTO, data: opts.proto });
...
export function parseContainer(buf) {
    ...
    const out = new Map();
    if (buf.length < 5 || dec.decode(buf.subarray(0, 4)) !== "POX1")
        return out;
    let off = 4;
