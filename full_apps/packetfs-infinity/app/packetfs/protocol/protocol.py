...
try:
    from packetfs import _bitpack  # type: ignore
...
class ProtocolEncoder:
    ...
    def pack_refs(self, out: bytearray, tier: int, refs: bytes, ref_bits: int) -> int:
        # Fallback to a calculation-free path when _bitpack is not present. We do not
        # emit any packed bits; we only maintain ref_count and CRC accounting so that
        # downstream logic relying on SYNC units and window metrics remains stable.
        if _bitpack is None:
            if self.sync.window_crc16:
                self._win_crc_accum += refs
            width = max(1, ref_bits // 8)
            self.ref_count = (self.ref_count + (len(refs) // width)) & 0xFFFFFFFF
            return 0
        if self.sync.window_crc16:
            self._win_crc_accum += refs
        bits = _bitpack.pack_refs(out, tier, refs, ref_bits)  # type: ignore[attr-defined]
        width = ref_bits // 8
        self.ref_count = (self.ref_count + (len(refs) // width)) & 0xFFFFFFFF
        return bits
