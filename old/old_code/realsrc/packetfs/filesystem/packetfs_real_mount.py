    ...
    def create_mount(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["mountpoint", "-q", str(self.mount_point)], capture_output=True
            )
    ...
    def compress_file(self, file_path: Path) -> CompressionStats:
        try:
            ...
            if file_hash in self.dedup_hashes:
                ...
                logger.info(
                    f"Deduplication: {file_path.name} matches {Path(existing_path).name}"
                )
                with self.lock:
            ...
            logger.info(
                f"Compressed {file_path.name}: {original_size:,} -> {compressed_size:,} bytes ({compression_ratio:.2f}x)"
            )
    ...
    def get_stats(self):
        with self.lock:
            ...
            return {
                ...
                "avg_compression": (
                    round(original_mb / compressed_mb, 2) if compressed_mb > 0 else 1.0
                ),
                "files_per_sec": (
                    round(self.metrics.total_files / uptime, 2) if uptime > 0 else 0
                ),
