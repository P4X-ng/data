...
class PacketFSRealFilePatternAnalyzer:
    ...
    def analyze_real_file(self, filepath: str) -> PatternAnalysis:
        ...
        total_chunks = len(data) // self.chunk_size + (
            1 if len(data) % self.chunk_size else 0
        )

        # Compression calculation
        original_size = len(data)
        compressed_size = (
            unique_patterns * self.chunk_size + total_chunks * 2
        )  # patterns + offsets
        compression_ratio = (
    ...
    def _analyze_pattern_trends(self):
        ...
        avg_discovery_time = (
            sum(a.pattern_discovery_time for a in self.analyses) / total_files
        )
        ...
        print(
            f"   Best compression: {best_compression.filename} ({best_compression.compression_ratio:.1f}:1)"
        )
        print(
            f"   Worst compression: {worst_compression.filename} ({worst_compression.compression_ratio:.1f}:1)"
        )
        print(
            f"   Fastest discovery: {fastest_discovery.filename} ({fastest_discovery.pattern_discovery_time*1000:.3f}ms)"
        )

        # File type analysis
        text_files = [a for a in self.analyses if self._is_likely_text(a.filename)]
        binary_files = [
            a for a in self.analyses if not self._is_likely_text(a.filename)
        ]

        if text_files:
            avg_text_compression = sum(a.compression_ratio for a in text_files) / len(
                text_files
            )
            print(f"   Text file avg compression: {avg_text_compression:.1f}:1")

        if binary_files:
            avg_binary_compression = sum(
                a.compression_ratio for a in binary_files
            ) / len(binary_files)
        ...
        for analysis in self.analyses[:5]:  # Top 5 files
            if analysis.top_patterns:
                ...
                pattern_preview = (
                    top_pattern[:16] + b"..." if len(top_pattern) > 16 else top_pattern
                )
                ...
                try:
                    ...
                    print(
                        f"   {analysis.filename}: '{preview_str}' appears {top_count} times"
                    )
                except:
                    # Display as hex
                    preview_hex = pattern_preview.hex()[:20] + "..."
                    print(
                        f"   {analysis.filename}: 0x{preview_hex} appears {top_count} times"
                    )

        # Calculate theoretical PacketFS performance
        total_transfer_time = sum(
            64 / (4 * 1024**5) for _ in range(total_files)
        )  # 64 bytes per file @ 4 PB/s
    ...
    def _is_likely_text(self, filename: str) -> bool:
        ...
        return any(filename.endswith(ext) for ext in text_extensions) or any(
            name in filename for name in text_names
        )
