...
class PacketFSSingleTargetScriptTransfer:
    ...
    def __init__(self):
        ...
        print(f"🖥️  Local system: {self.local_cpu_count} CPUs, {self.local_memory // (1024**3):.1f}GB RAM")
        print()
    
    def test_single_target_transfer(self, filename: str, file_size: int) -> TransferMeasurement:
        ...
        reconstructed_bytes = self._execute_script_with_measurement(temp_script_file, symbols, file_size)
...
'''
    ...
    def _execute_script_with_measurement(self, script_file: str, symbols: List[float], file_size: int) -> int:
            ...
            if result.returncode == 0:
                ...
                for line in output_lines:
                    if "Generated:" in line:
                        ...
                        try:
                            bytes_str = line.split("Generated:")[1].split("bytes")[0].strip().replace(',', '')
    ...
    def compare_with_traditional(self, measurement: TransferMeasurement) -> Dict[str, Any]:
    ...
    def run_reality_tests(self):
        ...
        for filename, size in test_files:
            ...
            print(f"   Efficiency: {measurement.file_size / measurement.network_bytes:,.0f}:1")
    ...
    def _analyze_breakeven_points(self):
        ...
        for measurement in self.measurements:
            reconstruction_rate = measurement.file_size / measurement.reconstruction_time  # bytes/sec
            
            print(f"\n{measurement.filename}:")
            print(f"   Reconstruction rate: {reconstruction_rate / 1_000_000:.1f} MB/s")
            print(f"   Network efficiency: {measurement.file_size / measurement.network_bytes:,.0f}:1")
            
            # When does script transfer win?
            if measurement.file_size / measurement.network_bytes > 100:  # >100:1 efficiency
            ...
            elif measurement.file_size / measurement.network_bytes > 10:  # >10:1 efficiency
