...
class PacketFSComputationalStorage:
    ...
    def read_calculated_file(self, filename: str, offset: int = 0, length: int = 1024) -> bytes:
    ...
    def demonstrate_infinite_storage(self):
        ...
        # Show storage stats
        virtual_total = sum(calc['size_bytes'] for calc in self.file_calculations.values())
        compression_ratio = virtual_total / self.TOTAL_PHYSICAL_STORAGE
        
        print()
        print("📊 STORAGE STATISTICS:")
        print(f"   • Physical storage: {self.TOTAL_PHYSICAL_STORAGE / 1_048_576:.1f} MB")
        ...
        data_sample = self.read_calculated_file("huge.data", offset=500_000_000, length=64)
