...
class PacketFSPureNativeTransfer:
    ...
    def run_pure_pfs_benchmarks(self):
        ...
        for filename, size in test_files:
            ...
            for speed in relevant_speeds:
                ...
                if data['speedup'] >= 1:
                    print(f"   vs {speed}: {data['speedup']:,.0f}x FASTER " +
    ...
    def _analyze_pure_pfs_performance(self):
        ...
        avg_compression = sum(t.compression_ratio for t in self.transfers) / len(self.transfers)
        ...
        print(f"   Fastest transfer: {smallest_transfer.filename} " +
              f"({smallest_transfer.symbol_transfer_time*1000:.6f}ms)")
        print(f"   Largest file: {largest_transfer.filename} " +
              f"({largest_transfer.file_size:,} bytes)")
        print(f"   Same transfer time: ALL files take ~{self.total_symbol_bytes / self.pfs_network_speed * 1000:.9f}ms")
        ...
        print(f"   Physics limit: {self.total_symbol_bytes / self.pfs_network_speed * 1000:.9f}ms")
