...
int main(int argc, char** argv){
    ...
    TxConfig cfg={0}; cfg.ifname=NULL; cfg.queue=0; cfg.zerocopy=1; cfg.blob_size=2ULL<<30; cfg.huge_dir="/dev/hugepages"; cfg.blob_name="pfs_stream_blob"; cfg.seed=0x12345678ULL; cfg.desc_per_frame=64; cfg.total_bytes=1ULL<<30; cfg.duration_s=0.0; cfg.align=64; cfg.verbose=1; cfg.corr_listen=NULL;
    for(int i=1;i<argc;i++){
        if(!strcmp(argv[i],"--ifname") && i+1<argc) cfg.ifname=argv[++i];
            ...
            printf("       [--seed S] [--desc-per-frame N] [--total-bytes B] [--duration S] [--align A] [--corr-listen HOST:PORT]\n");
            return 0;
        }
    }
    ...
    if(cfg.verbose) fprintf(stderr,"[TX] if=%s q=%u zerocopy=%d blob=%zu dir=%s name=%s dpf=%u total=%llu dur=%.2f align=%u\n",
        cfg.ifname,cfg.queue,cfg.zerocopy,(size_t)cfg.blob_size,cfg.huge_dir,cfg.blob_name,cfg.desc_per_frame,(unsigned long long)cfg.total_bytes,cfg.duration_s,cfg.align);
    ...
    while(!g_stop){
        ...
        if(reserved == 1){
            ...
            // Generate descriptors deterministically over blob
