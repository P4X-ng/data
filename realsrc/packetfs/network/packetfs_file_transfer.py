...
class PacketFSFileTransfer:
    ...
    def handle_file_request(self, client_sock: socket.socket, request: dict):
        ...
        for i, chunk in enumerate(chunks):
            ...
            self.send_message(client_sock, MSG_FILE_DATA, json.dumps(chunk_data).encode())
        ...
        self.send_message(client_sock, MSG_FILE_COMPLETE, json.dumps(complete_msg).encode())
    ...
    def handle_sync_request(self, client_sock: socket.socket, request: dict):
        ...
        if blob_id in self.sync_blobs:
            ...
            self.send_message(client_sock, MSG_SYNC_DATA, json.dumps(sync_data).encode())
        else:
            error_msg = json.dumps({'error': 'Sync blob not found'}).encode()
            self.send_message(client_sock, MSG_ERROR, error_msg)
    
    def connect_client(self, server_host: str, server_port: int = PFS_PORT) -> socket.socket:
    ...
    def request_file(self, server_host: str, remote_file_path: str, local_file_path: str):
            ...
            while True:
                ...
                if msg_type == MSG_FILE_DATA:
                    ...
                    if 'file_path' in chunk_data:  # Metadata
                        ...
                        print(f"📋 File: {chunk_data['file_size']} bytes, {chunk_data['chunks']} chunks")
                    else:  # Chunk data
    ...
    def print_stats(self):
        ...
        if elapsed > 0:
            throughput_mb = (self.stats['bytes_sent'] + self.stats['bytes_received']) / (1024 * 1024) / elapsed
