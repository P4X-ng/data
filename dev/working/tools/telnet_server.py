...
from packetfs_file_transfer import (
    PacketFSFileTransfer, PFS_PORT, PFS_MAGIC,
    MSG_HELLO, MSG_FILE_REQUEST, MSG_FILE_DATA, MSG_FILE_COMPLETE, MSG_ERROR
)
...
class TelnetSession:
    ...
    def welcome(self):
        ...
        self.send_line()
        self.send_line(f"{Colors.GREEN}╔═════════════════════════════════════════════════╗{Colors.RESET}")
        self.send_line(f"{Colors.GREEN}║ {Colors.BOLD}PacketFS Telnet Server{Colors.RESET}{Colors.GREEN}                         ║{Colors.RESET}")
        self.send_line(f"{Colors.GREEN}║ {Colors.RESET}rsync-like file transfer over telnet protocol{Colors.GREEN}   ║{Colors.RESET}")
        self.send_line(f"{Colors.GREEN}╚═════════════════════════════════════════════════╝{Colors.RESET}")
        self.send_line()
    ...
    def cmd_help(self, args: List[str]):
        ...
        self.send_line()
        self.send_line(f"  {Colors.CYAN}help{Colors.RESET}                  Show this help message")
        self.send_line(f"  {Colors.CYAN}ls [path]{Colors.RESET}             List directory contents")
        self.send_line(f"  {Colors.CYAN}pwd{Colors.RESET}                   Print working directory")
        self.send_line(f"  {Colors.CYAN}cd <path>{Colors.RESET}             Change directory")
        self.send_line(f"  {Colors.CYAN}mkdir <dir>{Colors.RESET}           Create directory")
        self.send_line(f"  {Colors.CYAN}rm <file|dir>{Colors.RESET}         Remove file or directory")
        self.send_line(f"  {Colors.CYAN}stat <file|dir>{Colors.RESET}       Show file information")
        self.send_line(f"  {Colors.CYAN}get <remote> [local]{Colors.RESET}  Download file")
        self.send_line(f"  {Colors.CYAN}put <local> [remote]{Colors.RESET}  Upload file")
        self.send_line(f"  {Colors.CYAN}transfer <src> <dst>{Colors.RESET}  Transfer file (rsync-like)")
        self.send_line(f"  {Colors.CYAN}sync <src> <dst>{Colors.RESET}      Sync directories (rsync-like)")
        self.send_line(f"  {Colors.CYAN}benchmark [size]{Colors.RESET}      Run performance test")
        self.send_line(f"  {Colors.CYAN}exit, quit, bye{Colors.RESET}       Exit session")
        self.send_line()
        self.send_line(f"{Colors.BOLD}Examples:{Colors.RESET}")
        self.send_line(f"  {Colors.YELLOW}transfer ~/file.txt /remote/path/{Colors.RESET}")
        self.send_line(f"  {Colors.YELLOW}get /etc/passwd local_passwd.txt{Colors.RESET}")
    ...
    def cmd_ls(self, args: List[str]):
        ...
        try:
            ...
            if os.path.exists(path):
                ...
                # Print headers
                self.send_line(f"{Colors.BOLD}{'Mode':<10} {'Size':>10} {'Modified':19} {'Name'}{Colors.RESET}")
                ...
                for entry in sorted(entries):
                    ...
                    try:
                        ...
                        self.send_line(f"{mode:<10} {size:>10} {mtime} {entry_formatted}")
    ...
    def cmd_stat(self, args: List[str]):
        ...
        try:
            ...
            if os.path.exists(full_path):
                ...
                self.send_line(f"{Colors.BOLD}Size: {Colors.RESET}{stat_info.st_size:,} bytes")
                self.send_line(f"{Colors.BOLD}Type: {Colors.RESET}{'Directory' if os.path.isdir(full_path) else 'File'}")
                self.send_line(f"{Colors.BOLD}Permissions: {Colors.RESET}{oct(stat_info.st_mode)[-3:]}")
                self.send_line(f"{Colors.BOLD}Modified: {Colors.RESET}{datetime.fromtimestamp(stat_info.st_mtime).strftime('%Y-%m-%d %H:%M:%S')}")
                self.send_line(f"{Colors.BOLD}Accessed: {Colors.RESET}{datetime.fromtimestamp(stat_info.st_atime).strftime('%Y-%m-%d %H:%M:%S')}")
                self.send_line(f"{Colors.BOLD}Created: {Colors.RESET}{datetime.fromtimestamp(stat_info.st_ctime).strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                self.send_line(f"{Colors.BOLD}Links: {Colors.RESET}{stat_info.st_nlink}")
    ...
    def cmd_get(self, args: List[str]):
        ...
        try:
            ...
            with open(remote_file, 'rb') as f:
                data = f.read()
                self.send_info(f"Sending file data (md5: {hashlib.md5(data).hexdigest()})")
    ...
    def cmd_transfer(self, args: List[str]):
        ...
        if '@' in source and ':' in source:
            ...
            self.send_warning("Remote source transfers require client-side implementation")
        elif '@' in destination and ':' in destination:
            # Local to remote transfer
            self.send_info(f"Local->Remote transfer: {source} -> {destination}")
            self.send_warning("Remote destination transfers require client-side implementation")
        else:
    ...
    def cmd_sync(self, args: List[str]):
        ...
        if '@' in source and ':' in source:
            ...
            self.send_warning("Remote destination sync requires client-side implementation")
        else:
            # Local to local sync
            try:
                # Get absolute paths
    ...
    def cmd_benchmark(self, args: List[str]):
        ...
        try:
            ...
            # Display results
            self.send_line()
            self.send_line(f"{Colors.CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{Colors.RESET}")
...
def main():
    ...
    parser.add_argument('--pfs-port', type=int, default=PFS_PORT, help='PacketFS server port')
