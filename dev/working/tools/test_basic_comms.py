...
def receiver_thread(ifname: str, test_duration: float, result_queue: queue.Queue):
    ...
    try:
        ...
        while time.time() - start_time < test_duration:
            try:
                ...
                if sync_info:
                    win, crc = sync_info
                    sync_frames += 1
                    print(f"📡 SYNC frame #{sync_frames}: window={win}, crc=0x{crc:04x}")
                else:
        ...
        avg_frame_size = total_frame_size / frames_received if frames_received > 0 else 0
...
def sender_test(ifname: str, frame_count: int, frame_interval: float = 0.001) -> TestResult:
    """Send test frames and measure performance."""
    enc = ProtocolEncoder(SyncConfig(window_pow2=6, window_crc16=True))  # 2^6 = 64 refs per window
...
def main():
    ...
    parser.add_argument("--interval", type=float, default=0.01, help="Interval between frames (seconds)")
    ...
    rx_thread = threading.Thread(
        ...
        args=(args.rx_iface, args.duration, result_queue)
    )
    ...
    if rx_result:
        ...
        print(f"⏱️  Total Time:         {max(tx_result.total_time, rx_result.total_time):.3f}s")
        ...
        success_rate = (rx_result.frames_received / tx_result.frames_sent) * 100 if tx_result.frames_sent > 0 else 0
