...
def generate_latency_comparison():
    ...
    ax1.scatter(
        x, typical_latencies, color="red", s=100, label="Typical Latency", zorder=5
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "protocol_comparison/latency_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_throughput_comparison():
    ...
    packetfs_efficiency = [
        t / (o + 0.001) for t, o in zip(packetfs_throughput, packetfs_overhead)
    ]
    ...
    plt.tight_layout()
    plt.savefig(
        "protocol_comparison/throughput_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_overhead_comparison():
    ...
    plt.tight_layout()
    plt.savefig(
        "protocol_comparison/overhead_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_use_case_comparison():
    ...
    ax.set_title(
        "Protocol Suitability by Use Case\n(10 = Excellent, 0 = Poor)",
        pad=20,
        fontsize=16,
        fontweight="bold",
    )
    ...
    plt.savefig(
        "protocol_comparison/use_case_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_performance_matrix():
    ...
    ax.set_title(
        "Protocol Performance Comparison Matrix\n(0-100 Scale, Higher = Better)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "protocol_comparison/performance_matrix.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_scaling_projections():
    ...
    ax1.plot(
        x, tcp_throughput, "s--", linewidth=2, markersize=6, label="TCP", color="blue"
    )
    ax1.plot(
        x, udp_throughput, "^:", linewidth=2, markersize=6, label="UDP", color="green"
    )
    ...
    ax2.plot(
        x, tcp_latency, "s--", linewidth=2, markersize=6, label="TCP", color="blue"
    )
    ax2.plot(
        x, udp_latency, "^:", linewidth=2, markersize=6, label="UDP", color="green"
    )
    ...
    # Frame rate scaling
    ax3.plot(
        x, packetfs_fps, "o-", linewidth=3, markersize=8, label="PacketFS", color="red"
    )
    ...
    ax4.bar(
        ...
        0.4,
        label="Throughput Advantage vs TCP",
        alpha=0.8,
    )
    ax4.bar(
        x + 0.2, latency_advantage, 0.4, label="Latency Advantage vs TCP", alpha=0.8
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "protocol_comparison/scaling_projections.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
