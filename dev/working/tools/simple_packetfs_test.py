...
class SimplePacketFSTester:
    ...
    def run_encode_test(
        self, payload_size: int, frame_count: int, window_size: int
    ) -> SimpleTestResult:
        """Run encoding performance test."""

        print(
            f"🚀 Testing encode: {frame_count} frames × {payload_size} bytes, window={window_size}"
        )

        config = SyncConfig(window_pow2=window_size.bit_length() - 1, window_crc16=True)
        ...
        for i in range(frame_count):
            ...
            encode_latencies.append(
                (encode_end - encode_start) * 1e6
        ...
        encode_mbps = (
            (total_bits / 1e6) / encode_total_time if encode_total_time > 0 else 0
        )
        bit_efficiency = (
            total_bits / (frame_count * payload_size * 8) if frame_count > 0 else 0
        )
        ...
        print(
            f"  ✅ Encode: {encode_fps:.0f} fps, {encode_mbps:.2f} Mbps, {result.encode_latency_avg_us:.2f}μs avg"
        )
        print(
            f"  ✅ Decode: {decode_fps:.0f} fps, {result.decode_latency_avg_us:.2f}μs avg"
        )
    ...
    def generate_summary_report(self, results: List[SimpleTestResult]) -> str:
        ...
        report.append(
            f"{'Test':<25} {'Payload':<8} {'Frames':<8} {'Encode FPS':<12} {'Encode Mbps':<12} {'Latency μs':<12}"
        )
        report.append("-" * 80)

        for result in results:
            report.append(
                f"{result.test_name:<25} {result.payload_size:<8} {result.frame_count:<8} "
        ...
        for size in sorted(payload_groups.keys()):
            ...
            avg_latency = statistics.mean(
                [r.encode_latency_avg_us for r in group_results]
            )

            report.append(
                f"{size:4d} bytes: {avg_fps:8.0f} fps, {avg_mbps:8.2f} Mbps, {avg_latency:8.2f} μs"
            )
