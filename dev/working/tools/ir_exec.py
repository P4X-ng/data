...
def run_scheduler_windows(ll_path: str, ops_only: bool = False) -> Dict[str, Any]:
    sched = WindowedScheduler(window_pow2=16)
    t0 = time.perf_counter()
    result_check = None
    if ops_only:
...
def main(argv: list[str]) -> int:
    ...
    if args.windows:
        try:
            win_res = run_scheduler_windows(ll_file, ops_only=(args.mode == "analyze"))
            out["windows"] = win_res
        except RuntimeError as e:
