    ...
    def send_burst(self, payloads: List[bytes]) -> Tuple[float, List[float]]:
        ...
        for payload in payloads:
            ...
            if frame_len > 14 + payload_len:
                # Zero-fill padding
                self.frame_buffer[14 + payload_len : frame_len] = b"\x00" * (
...
class HighPerformanceReceiver:
    ...
    def __init__(self, ifname: str):
        ...
        self.socket = socket.socket(
            socket.AF_PACKET, socket.SOCK_RAW, socket.htons(ETH_P_PFS)
        )
        self.socket.bind((ifname, 0))
    ...
    def receive_burst(
        self, expected_count: int, max_wait_time: float = 10.0
        ...
        while (
            frames_received < expected_count
...
def run_performance_test(
    ...
    print(
        f"🏃 Running {test_name}: {frame_count} frames, {payload_size} refs, burst={burst_size}"
    )
    ...
    # Split into bursts
    payload_bursts = [
        payloads[i : i + burst_size] for i in range(0, len(payloads), burst_size)
    ]
    ...
    def receiver_worker(result_queue: queue.Queue):
        frames_received, recv_times, sync_count, crc_errors = receiver.receive_burst(
            frame_count
        )
    ...
    total_bits = (
        frames_received * payload_size * 8
    )  # Assume 8 bits per ref for simplicity
    mbits_per_second = (total_bits / 1_000_000) / total_time if total_time > 0 else 0
    refs_per_second = (
        (frames_received * payload_size) / total_time if total_time > 0 else 0
    )
    ...
    # Protocol metrics
    sync_rate = (
        (recv_result["sync_count"] / frames_received * 100)
        if frames_received > 0
        else 0
    )

    print(
        f"  ✅ Complete: {frames_received}/{frame_count} frames, {frames_per_second:.1f} fps, {mbits_per_second:.2f} Mbps"
    )
...
def print_benchmark_results(results: List[PerformanceMetrics]):
    ...
    print(
        f"{'Test Name':<20} {'Frames':<8} {'Payload':<8} {'Burst':<6} {'FPS':<10} {'Mbps':<10} {'Refs/s':<12} {'Lat(μs)':<10} {'CPU%':<8} {'Mem(MB)':<8}"
    )
    print("-" * 120)

    for result in results:
        print(
            f"{result.test_name:<20} {result.frame_count:<8} {result.payload_size:<8} {result.burst_size:<6} "
...
def main():
    ...
    parser.add_argument(
        "--intensive", action="store_true", help="Run intensive benchmark (more tests)"
    )
    ...
    print(
        f"🔧 Config: {len(config.frame_counts)} frame counts, {len(config.payload_sizes)} payload sizes, {len(config.burst_sizes)} burst sizes"
    )
    ...
    test_count = 0
    total_tests = (
        len(config.frame_counts)
        * len(config.payload_sizes)
        * len(config.burst_sizes)
        * config.iterations
    )
